...
I love a great developer experience (DX) as much as the next person, which is why I ended up going too far with my Gatsby site's components. The entire four pages for the site were written in squeaky clean markup because I spent a lot of time on a component API to avoid using Emotion's css prop in my pages. Instead I implelemented components to accept style props, like a component library might do, ladeling on the complexity.

But I am not building a design system (yet) so I decided to explore what tooling options were available to spare me from (a) spending too much time writing component code and (b) the horror of writing untidy code for my pages, with css prop styles all over the place to tweak spacing here and there while chasing pixel-perfection. The standout options were Theme UI and Tailwind CSS.
...
But seeing a troupe of classnames lined up makes me feel uneasy. Given the glowing feedback Tailwind receives, I am sure the unease subsides with acclimitization, and I fully intend to test that theory at some point. But for my personal site I need easy to read and maintainable code; a testing ground for all the shiny new things must be free from chaos for quick iteration.

There was also the requirement to learn the Tailwind API rather than build on the CSS skills that I want to develop. It is too far removed from plain CSS for me to develop my skills in that area.

## Eureka

Then came the lightbulb moment: Maybe a new styling tool wasn't the right solution. I liked the structured theme of Theme UI, and the well considered design system of Tailwind, but didn't want to use a tool that abstracted away from plain CSS too much. I was already on good terms with Emotion after the Gatsby site I spent so much time iterating upon and learning from, so why not take some features from Theme UI and Tailwind and implement them myself with Emotion? That way I can stay closer to vanilla CSS and as a bonus get to explore how the other tools work.

## But What About Reducing Complexity?

In the outset I wanted to avoid two things that bothered me while implementing the previous version of my personal site:

1.  Spending too much time writing component code
2.  Writing untidy code for my pages

The search for technical solutions gave rise to an unexpected conclusion. While tools like Theme UI and Tailwind CSS are great for speedy development and provde a fantastic API, in the interest of growing my CSS skills I found that I wasn't really confortable with the level of abstraction these tools bring.

My problems could be better soloved with a change in mindset. In this case, I am building a personal site and blog with a handful of pages. It isn't justifiable to spend time on creating a clean component API. For the few basic pages I need, using the css prop for tweaks here and there in the markup isn't much of a problem.
...
And there we have it. To recap, we have a Next.js configuration with great DX featuring:
...
The big takeaway I have learned from this is to use a suitable approach for the task at hand, and not create complexity where it isn't justifiable. Sometimes we can get carried away with creating solutions for problems that can be solved in other ways, which only becomes apparaent when we step away from the code and look at the bigger picture.

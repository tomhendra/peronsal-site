...
To create a secure connection between Cloudflare and Vercel, set Cloudflare's
...
If succesfful you should see `HTTP/1.1 404 Not Found` returned, which means you
...
Now all should be up and running with a proxy-enabled Clouflare site. Vercel
recommends to avoid using the proxy due to performance degredation, but in my
